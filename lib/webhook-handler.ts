import { KNOWN_APPS, parseExternalReference, planDias, expiryFromDias } from './mp-contract.ts';

// ── Injectable deps (slice C: route-level tests under plain node) ──────
// handleWebhook holds the full policy; app/api/webhook/route.ts POST is a
// thin wrapper. Tests stub fetch + Supabase with no network (G7). HTTP
// contract is unchanged: same statuses, same messages, same ordering
// (env fail-loud before parsing).
//
// NOTE: this file lives in lib/ (not app/api/) on purpose — Next.js
// requires route.ts files to export ONLY HTTP methods (GET/POST/...).
// Exporting handleWebhook from a route file breaks `next build` type
// checking, so the handler is a plain reusable module here.
